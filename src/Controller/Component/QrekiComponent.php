...
class QrekiComponent extends Component
{
    #=========================================================================
    # 六曜算出関数
    #
    ...
    function get_rokuyou($year,$mon,$day)
    {
    	...
    	list($q_yaer,$uruu,$q_mon,$q_day) = self::calc_kyureki($year,$mon,$day);
    ...
    function calc_kyureki($year,$mon,$day)
    {
      ...
      for($i=0;$i<13;$i++){
    	...
    	$tm0 = self::YMDT2JD($year,$mon,$day,0,0,0);
    #-----------------------------------------------------------------------
    # 計算対象の直前にあたる二分二至の時刻を求める
    # chu[0,0]:二分二至の時刻  chu[0,1]:その時の太陽黄経
    #-----------------------------------------------------------------------
    	list($chu[0][0],$chu[0][1]) = self::before_nibun($tm0);
    ...
    #-----------------------------------------------------------------------
    	for($i=1;$i<4;$i++){
    		list($chu[$i][0],$chu[$i][1]) = self::calc_chu($chu[$i-1][0]+32.0);
    	}
    ...
    # +33日にして再実行させる。
    		if( abs( self::int($saku[$i-1])-self::int($saku[$i]) ) <= 26.0 ){
    ...
    #-----------------------------------------------------------------------
    	if( self::int($saku[1]) <= self::int($chu[0][0]) ){
    		for($i=0;$i<5;$i++){
    			$saku[$i]=$saku[$i+1];
    		}
    		$saku[4] = self::calc_saku($saku[3]+35.0);
    	}
    ...
    #-----------------------------------------------------------------------
    	elseif( self::int($saku[0]) > self::int($chu[0][0]) ){
    		for($i=4;$i>0;$i--){
    			$saku[$i] = $saku[$i-1];
    		}
    		$saku[0] = self::calc_saku($saku[0]-27.0);
    	}
    ...
    # lap=0:平月  lap=1:閏月
    #-----------------------------------------------------------------------
    	if(self::int($saku[4]) <= self::int($chu[3][0]) ){
    		$lap=1;
    	}else{
    		$lap=0;
    	}
    ...
    #-----------------------------------------------------------------------
    	...
    	for($i=1;$i<5;$i++){
    		if($lap == 1 && $i !=1 ){
    			if( self::int($chu[$i-1][0]) <= self::int($saku[$i-1]) || self::int($chu[$i-1][0]) >= self::int($saku[$i]) ){
    				$m[$i-1][0] = $m[$i-2][0];
    				$m[$i-1][1] = 1;
    ...
    #-----------------------------------------------------------------------
    	$state=0;
    	for($i=0;$i<5;$i++){
    		if(self::int($tm0) < self::int($m[$i][2])){
    			$state=1;
    			break;
    		}elseif(self::int($tm0) == self::int($m[$i][2])){
    			$state=2;
    			break;
    		}
    	}
    	...
    	$kyureki[1]=$m[$i][1];
    	$kyureki[2]=$m[$i][0];
    	$kyureki[3]=self::int($tm0)-self::int($m[$i][2])+1;
    ...
    #-----------------------------------------------------------------------
    	...
    	$rm_sun0 = 30.0*self::int($rm_sun/30.0);
    ...
    #-----------------------------------------------------------------------
    	...
    	$t=$t + ($tm1-2451545.0) / 36525.0;
    	$rm_sun = self::LONGITUDE_SUN( $t );
    	$rm_sun0 = 90 * self::int($rm_sun/90.0);
    ...
    # 摂動項の計算
    #-----------------------------------------------------------------------
    	$ang = self::NORMALIZATION_ANGLE(  31557.0 * $t + 161.0 );
    	$th =       .0004 * cos( $k*$ang );
    ...
    # 摂動項の計算
    #-----------------------------------------------------------------------
      $ang = self::NORMALIZATION_ANGLE( 2322131.0  * $t + 191.0  );
       $th =      .0003 * cos( $k*$ang );
    ...
    function YMDT2JD($year,$month,$day,$hour,$min,$sec){
    	...
    	$jd  = self::int( 365.25 * $year );
    	$jd += self::int( $year / 400.0 );
    	$jd -= self::int( $year / 100.0 );
    	$jd += self::int( 30.59 * ( $month-2.0 ) );
    	$jd += 1721088;
    	$jd += $day;
    ...
    function JD2YMDT($JD)
    {
    	...
    	$x0 = self::int( $JD+68570.0);
    	$x1 = self::int( $x0/36524.25 );
    	$x2 = $x0 - self::int( 36524.25*$x1 + 0.75 );
    	$x3 = self::int( ( $x2+1 )/365.2425 );
    	$x4 = $x2 - self::int( 365.25*$x3 )+31.0;
    	$x5 = self::int( self::int($x4) / 30.59 );
    	$x6 = self::int( self::int($x5) / 11.0 );
    ...
    #-----------------------------------------------------------------------
    	...
    	#
    	$rm_sun_today0   = 15.0 * self::int($rm_sun_today / 15.0);
    	$rm_sun_tommorow0 = 15.0 * self::int($rm_sun_tommorow / 15.0);
